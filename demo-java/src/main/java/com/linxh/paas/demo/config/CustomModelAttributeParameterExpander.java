...
import com.linxh.paas.demo.anno.SwaggerGroup;
 ...
 *
 * @author lin
 * @see CustomModelAttributeParameterExpander##propertyDescriptors(ExpansionContext)
 * <p>
 * 支持一个DTO在不同组下展现不同的属性
 */
@Component
...
public class CustomModelAttributeParameterExpander extends ModelAttributeParameterExpander {
    ...
    public List<Parameter> expand(ExpansionContext context) {
        ...
        Set<PropertyDescriptor> propertyDescriptors = propertyDescriptors(context);
    ...
    private Set<PropertyDescriptor> propertyDescriptors(ExpansionContext context) {
        Class<?> clazz = context.getParamType().getErasedType();
        try {
            Optional<SwaggerGroup> swaggerGroupOptional = context.getOperationContext().findAnnotation(SwaggerGroup.class);
            return customPropertyDescriptors(swaggerGroupOptional, clazz, FluentIterable.from(getBeanInfo(clazz).getPropertyDescriptors())
                    .toSet());
    ...
    private Set<PropertyDescriptor> customPropertyDescriptors(Optional<SwaggerGroup> swaggerGroupOptional, Class<?> clazz, ImmutableSet<PropertyDescriptor> propertyDescriptors) {
        Set<PropertyDescriptor> set = new HashSet<>();
        for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
            Field field = FieldUtils.getDeclaredField(clazz, propertyDescriptor.getName(), true);
            if (null == field) {
                continue;
            }
            // 根据分组展现,接口方法添加@SwaggerGroup则进行校验,属性添加不添加@SwaggerGroup时表示直接展示
            boolean isAdd = checkSwaggerGroup(swaggerGroupOptional, field);
            // TODO 隐藏属性不展现[待完善]
            if (isAdd) {
    ...
    private boolean checkSwaggerGroup(Optional<SwaggerGroup> swaggerGroupOptional, Field field) {
        if (!swaggerGroupOptional.isPresent()) {
            return true;
        }
        SwaggerGroup swaggerGroup = field.getAnnotation(SwaggerGroup.class);
        if (null == swaggerGroup) {
            return true;
        }
        Class<?>[] methodClasses = swaggerGroupOptional.get().value();
        Class<?>[] fieldClasses = swaggerGroup.value();
        return compare(methodClasses, fieldClasses);
    }

    private boolean compare(Class<?>[] classes1, Class<?>[] classes2) {
        Class<?>[] maxClasses = classes1.length > classes2.length ? classes1 : classes2;
        Class<?>[] minClasses = classes1.length > classes2.length ? classes2 : classes1;
        Set<Class<?>> set = new HashSet<>(Arrays.asList(maxClasses));
        boolean flag = false;
        for (Class<?> clazz : minClasses) {
            if (!set.add(clazz)) {
                flag = true;
                break;
            }
